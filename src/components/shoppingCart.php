<div
    ...
    class="col-span-12 md:col-span-3 h-screen top-0 sticky px-2 pt-5 bg-amber-50">
        ...
        <template x-for="(item, index) in cart" :key="index">
            <div class="grid grid-cols-[7fr_2fr_3fr] px-1 py-1 gap-2 border-gray-500">
                <span class="text-left">
                    <span x-text="item.itemName"></span> @ <span x-text="item.itemPrice"></span> PHP
                </span>

                <span class="text-center">
                    x<span x-text="item.itemQuantity"></span>
                </span>

                <span class="text-right">
                    <span x-text="item.itemPrice * item.itemQuantity"></span> PHP
                </span>
            </div>
        </template>
    </div>
    ...
    <h2 class="my-1 px-5 text-xl font-semibold text-right">
        ...
        <span class="font-bold">
            PHP <span x-text="totalAmount.toFixed(2)"></span>
        </span>
    </h2>
    ...
    <div class="flex flex-col mr-2 items-end space-y-3">
        <input
            type="number"
            id="userPayAmount"
            x-model.number="userPayAmount"
            min="0"
            value=""
            class="px-3 py-1 w-5/8 border-2 rounded-xl focus:outline-none focus:ring-0">
        ...
        <div class="flex flex-row space-x-3">
            ...
            <button
                class="px-3 py-1 border-2 border-black rounded-4xl hover:bg-black hover:text-amber-50 duration-150 cursor-pointer select-none"
                @click="checkOutCart(userPayAmount)">
                Check Out</button>
        </div>
    </div>
</div>